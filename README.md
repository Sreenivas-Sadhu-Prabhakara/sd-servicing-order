...
| POST | `/v1/servicing-order-procedure/initiate` | Initiate |
| GET | `/v1/servicing-order-procedure` | Retrieve (list) |
| GET | `/v1/servicing-order-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/servicing-order-procedure/{crId}/update` | Update |
| PUT | `/v1/servicing-order-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/servicing-order-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-servicing-order:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-servicing-order ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-servicing-order` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
